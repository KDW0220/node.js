...
router.get('/list', function(req, res, next) {
    ...
    //전체데이타갯수
    var sql='select count(*) total from tbl_food';
    db.get().query(sql, function(err, rows){
      var total=rows[0].total;
  
      sql=`select *,format(price,0) fprice from tbl_food order by code desc limit ${start},4`;
      db.get().query(sql, function(err, rows){
...
router.get('/insert',function(req,res){
    var sql="select max(code) mcode from tbl_food"
    db.get().query(sql,function(err, rows){
...
router.post('/insert',function(req,res){
    ...
    var sql="insert into tbl_food(code,title,price,image) values(?,?,?,?)";
